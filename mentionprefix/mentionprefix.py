...
class MentionPrefix(commands.Cog):
    """Ping le bot pour voir ses prefixes."""
    ...
    __version__ = "1.1.0"
    __author__ = ["Draper", "TrustyJAID", "Francais par UnicornCorp"]
    ...
    async def commands_mentiontoggle(self, ctx: commands.Context):
        """Toggle si le fait de ping le bot envoie le help."""
        if ctx.guild.id in self.disable_in:
            self.disable_in.discard(ctx.guild.id)
            await ctx.send(_("Mentionner le bot lui fera envoyer le message d'aide."))
        else:
            self.disable_in.add(ctx.guild.id)
            await ctx.send(_("Mentioninner le bot ne lui fera plus envoyer le message d'aide."))
    ...
    async def on_message_without_command(self, message: discord.Message):
        ...
        verb = _("Mon préfix ici est :") if len(prefix_list) == 1 else _("Mon préfix ici est :")
        if help_command:
