...
class Away(commands.Cog):
    ...
    async def make_embed_message(self, author, message, state=None):
        ...
        if state == "away":
            ...
            em.set_author(name=f"{author.display_name} est AFK", icon_url=avatar)
        elif state == "idle":
            em = discord.Embed(description=message, color=color)
            em.set_author(name=f"{author.display_name} est absent", icon_url=avatar)
        elif state == "dnd":
            em = discord.Embed(description=message, color=color)
            em.set_author(name=f"{author.display_name} est en 'ne pas déranger'", icon_url=avatar)
        elif state == "offline":
            em = discord.Embed(description=message, color=color)
            em.set_author(name=f"{author.display_name} est offline", icon_url=avatar)
        elif state == "gaming":
            em = discord.Embed(description=message, color=color)
            em.set_author(
                name=f"{author.display_name} joue à {author.activity.name}", icon_url=avatar,
            )
            ...
            em = discord.Embed(description=message, color=color)
            em.set_author(
                name=f"{author.display_name} joue à {status[0].name}", icon_url=avatar,
            )
            ...
            artist_title = f"{author.activity.title} par " + ", ".join(
            ...
            em.set_author(
                name=f"{author.display_name} est en train d'écouter",
                icon_url=avatar,
                url=url,
            )
            em.description = (
            ...
            em.title = getattr(author.activity, "details", None)
            em.set_author(
                name=f"{author.display_name} stream {author.activity.name}", icon_url=avatar,
            )
            ...
            em.title = getattr(author.activity, "details", None)
            em.set_author(
                name=f"{author.display_name} stream {activity[0].name}", icon_url=avatar,
            )
        else:
            em = discord.Embed(color=color)
            em.set_author(name="{} est absent".format(author.display_name), icon_url=avatar)
    ...
    async def make_text_message(self, author, message, state=None):
        ...
        if state == "away":
            msg = f"{author.display_name} est AFK"
        elif state == "idle":
            msg = f"{author.display_name} est absent"
        elif state == "dnd":
            msg = f"{author.display_name} est en 'ne pas déranger'"
        elif state == "offline":
            msg = f"{author.display_name} est offline"
        elif state == "gaming":
            msg = f"{author.display_name} joue à {author.activity.name}"
        elif state == "gamingcustom":
            status = [c for c in author.activities if c.type == discord.ActivityType.playing]
            msg = f"{author.display_name} est en train de jouer {status[0].name}"
        elif state == "listening":
            ...
            msg = f"{author.display_name} écoute {artist_title}\n{currently_playing}"
        elif state == "listeningcustom":
            ...
            msg = f"{author.display_name} stream {author.activity.url}"
        elif state == "streamingcustom":
            status = [c for c in author.activities if c.type == discord.ActivityType.streaming]
            msg = f"{author.display_name} stream {status[0].url}"
        else:
            msg = f"{author.display_name} est AFK"

        if message != " " and state != "listeningcustom":
            msg += f" et a mis ce message : `{message}`"
        elif message != " " and state == "listeningcustom":
            msg += f"\n\nMessage customisé: `{message}`"
    ...
    async def away_(self, ctx, delete_after: Optional[int] = None, *, message: str = None):
        ...
        if delete_after is not None and delete_after < 5:
            return await ctx.send("Met un minimum de 5 secondes sur le délai.")
    ...
    async def idle_(self, ctx, delete_after: Optional[int] = None, *, message: str = None):
        ...
        if mess:
            ...
            msg = "Le bot ne repondra plus pour vous."
        else:
            if message is None:
    ...
    async def offline_(self, ctx, delete_after: Optional[int] = None, *, message: str = None):
        ...
        if mess:
            ...
            msg = "Le bot ne répondra plus pour vous."
        else:
            if message is None:
    ...
    async def gaming_(self, ctx, game: str, delete_after: Optional[int] = None, *, message: str = None):
        ...
        if game.lower() in mess:
            ...
            msg = f"Le bot ne repondra plus pour vous."
        else:
            if message is None:
            ...
            await self.config.user(author).GAME_MESSAGE.set(mess)
            msg = f"Le bot répondra pour toi."
        await ctx.send(msg)
