...
class Confession(BaseCog):
    ...
    async def confess(self, ctx, *, confession):
        ...
        if len(user_guilds) == 0:
            await ctx.author.send("Tu n'es pas dans un serveur avec un salon de confession!")
        if len(user_guilds) == 1:
            ...
            for guild in user_guilds:
                embed = discord.Embed()
                embed.title = "Que veux-tu confesser ?"
    ...
    async def send_confession(self, ctx, confession_guild, confession):
        ...
        if not confession_room:
            return await ctx.author.send("La salle de confession n'existe pas ...?")

        try:
            await ctx.bot.send_filtered(destination=confession_room, content=confession)
        except discord.errors.Forbidden:
            return await ctx.author.send("Probleme de permission ou erreur.")
            

        await ctx.author.send("Ton message est envoyé.")
