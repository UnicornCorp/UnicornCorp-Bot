...
DEFAULT_CONF = {
    "rouge": {"emoji": None, "colour": None},
    "orange": {"emoji": None, "colour": None},
    "vert": {"emoji": None, "colour": None},
}

DEFAULT_FOOTER = (
    "Si le bot reponds correctement, les chiffres n'ont pas d'importance\nScale: Excellent | "
    "Bon | Ca passe | Mauvais | Gnééh ?"
)
...
class AnotherPingCog(commands.Cog):
    ...
    __version__ = "1.1.6"
    __author__ = "Vexed#3211 -- FR pour et par UnicornCorp"
    ...
    async def ping(self, ctx: commands.Context):
        ...
        if use_embed:
            ...
            embed.add_field(name="Discord WebSocket", value=box(f"{ws_latency} ms", "py"))
            ...
            msg = f"**{title}**\nDiscord WebSocket: {ws_latency} ms"
            start = monotonic()
        ...
        if use_embed:
            ...
            embed.set_field_at(0, name="Discord ebSocketS", value=f"{ws_latency_text}{extra}")
            extra = box(f"{m_latency} ms", "py")
            embed.add_field(name="Message envoyé", value=f"{m_latency_text}{extra}")
            embed.colour = colour
    ...
    def _get_latency_text(self, ws_latency: int, m_latency: int, settings: Cache, emojis: bool):
        if ws_latency < 50:
            ...
            ws_latency_text = f"{settings.green.emoji} Good" if emojis else "Bon"
        elif ws_latency < 250:
            ws_latency_text = f"{settings.orange.emoji} Alright" if emojis else "Ca passe"
        elif ws_latency < 500:
            ws_latency_text = f"{settings.red.emoji} Bad" if emojis else "Mauvais"
        else:
            ws_latency_text = f"{settings.red.emoji} Very Bad" if emojis else "Gnééh ?"

        if m_latency < 75:
            m_latency_text = f"{settings.green.emoji} Excellent" if emojis else "Excellent"
        elif m_latency < 225:
            m_latency_text = f"{settings.green.emoji} Good" if emojis else "Bon"
        elif m_latency < 350:
            m_latency_text = f"{settings.orange.emoji} Alright" if emojis else "Ca passe"
        elif m_latency < 600:
            m_latency_text = f"{settings.red.emoji} Bad" if emojis else "Mauvais"
        else:
            m_latency_text = f"{settings.red.emoji} Very Bad" if emojis else "Gnééh ?"
